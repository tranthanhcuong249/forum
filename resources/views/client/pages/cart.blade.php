		...
		<div class="container py-xl-4 py-lg-2">
			...
			<div class="checkout-right">
				<h4 class="mb-sm-4 mb-3">Bạn có tổng cộng:
					<span>{{ count(Cart::getContent()) }} sản phẩm</span>
				</h4>
							...
							@foreach($cart as $key => $value)
								<tr class="rem1">
									...
									<td class="invert-image">
										<a href="#">
                                            <img src="img/upload/product/{{ $value->attributes->img }}" height="100" alt="{{ $value->name }}" class="img-responsive">
                                        </a>
									</td>
									<td class="invert">
										<div class="quantity">
											<div class="form-group">
                                                <input type="number" name="qty" class="form-control qty" value="{{ $value->quantity }}" min='1' data-id="{{ $value->id }}">
                                            </div>
										</div>
									</td>
									...
									<td class="invert">
										<div class="rem">
											<div class="close1" data-id="{{ $value->id}}"></div>
										</div>
									</td>
								</tr>
								<?php $i++; ?>
